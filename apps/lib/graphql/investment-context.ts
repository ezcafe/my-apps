...
export type InvestmentGraphQLContext = {
  requestId: string;
...
export async function createInvestmentGraphQLContext(
  requestId: string,
  responseHeaders: Headers,
  request?: Request,
): Promise<InvestmentGraphQLContext> {
  ...
  if (auth.method === "api_key" && auth.apiTokenAppKey !== "investment") {
    return {
      requestId,
      responseHeaders,
      request,
...
export function requireInvestmentAuth(ctx: InvestmentGraphQLContext): string {
  if (!ctx.userSub) throw new Error("UNAUTHORIZED");
  return ctx.userSub;
}

export function requireInvestmentWriteScope(ctx: InvestmentGraphQLContext): void {
  if (!hasWriteScope(ctx.scopes)) throw new Error("FORBIDDEN");
}

export function requireInvestmentWorkspace(ctx: InvestmentGraphQLContext): {
  userSub: string;
  workspaceId: string;
} {
...
export function requireInvestmentWriteWorkspace(ctx: InvestmentGraphQLContext): {
  userSub: string;
  workspaceId: string;
} {
  requireInvestmentWriteScope(ctx);
  return requireInvestmentWorkspace(ctx);
}
