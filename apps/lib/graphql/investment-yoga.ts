...
import { investmentTypeDefs } from "@/lib/graphql/investment-typeDefs";
...
import { createInvestmentGraphQLContext } from "@/lib/graphql/investment-context";
...
const INVESTMENT_WORKSPACE_COOKIE = "ctx_workspace_investment";
...
export function investmentResponseCacheSessionKey(request: Request): string | null {
  ...
  const workspace =
    getCookieValue(cookie, INVESTMENT_WORKSPACE_COOKIE) ?? "none";
  const sessionToken =
...
const yoga = createYoga({
  schema,
  graphqlEndpoint: "/api/graphql/investment",
  landingPage: false,
  context: async ({ request }) => {
    ...
    return createInvestmentGraphQLContext(requestId, responseHeaders, request);
  },
  maskedErrors: process.env.NODE_ENV !== "development",
  plugins: [
    useResponseCache({
      session: investmentResponseCacheSessionKey,
      ttl: 0,
...
export function handleInvestmentGraphQL(
  request: Request,
