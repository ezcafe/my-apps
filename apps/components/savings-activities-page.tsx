...
import { savingsActivitiesQueryOptions } from "@/lib/savings-query-options";

export function SavingsActivitiesPage() {
  const { workspaceReady } = useSavingsWorkspace();
  const { formatDate } = useFormatDate();
  const activitiesQuery = useQuery({
    ...savingsActivitiesQueryOptions({ limit: 100 }),
              ...
              <tbody className="divide-y divide-border">
                {activitiesQuery.data.items.map((row) => (
                  <tr key={row.id}>
                    ...
                    <td className="py-2.5 pr-4">{row.accountName}</td>
