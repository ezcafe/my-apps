...
const LoanEditPageLazy = dynamic(
  () =>
    import("@/components/loan-edit-form").then((mod) => ({
      default: mod.LoanEditPage,
    })),
  {
    loading: () => <LoanEditPageSkeleton />,
  },
);

export default async function LoanEditRoute({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  const { id } = await params;
  return <LoanEditPageLazy loanId={id} />;
}
